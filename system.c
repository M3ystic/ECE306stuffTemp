...
//------------------------------------------------------------------------------
extern char CLIENT_tx_buf[BUFFERSIZE];
extern char CLIENT_rx_buf[BUFFERSIZE];
extern char IOTmodule_rx_buf[BUFFERSIZE];
extern char IOTmodule_tx_buf[BUFFERSIZE];
extern unsigned int CLIENT_tx;
extern unsigned int CLIENT_rx;
unsigned int IOTmodule_tx;
unsigned int IOTmodule_rx;
extern int IOT_RECIEVEDFLAG;
extern int CLIENT_RECIEVEDFLAG;
//------------------------------------------------------------------------------
// Command Flags
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
extern char stored_ip[IPLENGTH];
extern char stored_command[COMMANDSIZE];
...
char change;
char BL_Stop[LINE_LEN] = " BL Stop! "; // "BL Stop" fits within 10 characters, null terminator added.
//------------------------------------------------------------------------------
// Function Prototypes
//------------------------------------------------------------------------------
...
void initialize_hardware(void) {
    ...
    memset(IOTmodule_rx_buf, '\0', sizeof(IOTmodule_rx_buf));
    memset(CLIENT_rx_buf, '\0', sizeof(CLIENT_rx_buf));
    memset(IOTmodule_tx_buf, '\0', sizeof(IOTmodule_tx_buf));
    memset(CLIENT_tx_buf, '\0', sizeof(CLIENT_tx_buf));
...
// Handle received data from IOT and CLIENT
void handle_received_data(void) {
    if (IOT_RECIEVEDFLAG == ENABLED) {
        parse_and_execute_commands(IOTmodule_rx_buf);
        strcpy(CLIENT_tx_buf, IOTmodule_rx_buf);
        IOTmodule_rx = RESET_STATE;
        CLIENT_rx = RESET_STATE;
        IOTmodule_tx = RESET_STATE;
        CLIENT_tx = RESET_STATE;
        UCA1TXBUF = CLIENT_tx_buf[CLIENT_tx++];
        UCA1IE |= UCTXIE;
        display_changed = TRUE;
        IOT_RECIEVEDFLAG = DISABLED;
    }

    if (CLIENT_RECIEVEDFLAG == ENABLED) {
        strcpy(IOTmodule_tx_buf, CLIENT_rx_buf);
        UCA0TXBUF = IOTmodule_tx_buf[IOTmodule_tx++];
        UCA0IE |= UCTXIE;
        CLIENT_RECIEVEDFLAG = DISABLED;
    }
...
void clear_buffers(void) {
    if (A1TX_complete_flag) {
        memset(CLIENT_tx_buf, RESET_STATE, sizeof(CLIENT_tx_buf));
        A1TX_complete_flag = FALSE;

        if (CLIENT_tx == 0) {
            memset(CLIENT_rx_buf, RESET_STATE, sizeof(CLIENT_rx_buf));
        }
    }

    if (A0TX_complete_flag) {
        memset(IOTmodule_tx_buf, RESET_STATE, sizeof(IOTmodule_tx_buf));
        A0TX_complete_flag = FALSE;

        if (IOTmodule_tx == RESET_STATE) {
            memset(IOTmodule_rx_buf, RESET_STATE, sizeof(IOTmodule_rx_buf));
        }
    }
}
